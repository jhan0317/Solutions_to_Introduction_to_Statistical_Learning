...
preds = predict(fit,newdata=list(age=age.grid),se=TRUE)  # standard errors
se.bands=cbind(preds$fit+2*preds$se.fit,preds$fit,preds$fit-2*preds$se.fit)
...
( -11.983) ^2 # The square of the t-statistics are equal to the F-statistics
# [1] 143.5923
...
pfit[1:5]
#           1            2            3            4            5 
#9.826427e-09 7.577844e-08 4.746699e-07 2.449201e-06 1.055188e-05 

se.bands.logit = cbind(preds$fit+2*preds$se.fit, preds$fit-2*preds$se.fit)
se.bands = exp(se.bands.logit)/(1+exp(se.bands.logit))
se.bands[1:5]
# [1] 0.002112586 0.002769461 0.003572811 0.004538493 0.005680373
...
points(jitter(age), I((wage>250)/5),cex=.5,pch="|",col ="darkgrey") #Rug plots
...
fit=loess(wage~age,span=.2,data=Wage) # each neighborhood consists of 20 % or 50 % of the observations
...
plot.gam(gam1, se=TRUE, col="red")   # Does not work
...
# better than a GAM that does not include year at all (p-value = 0.00014). 
# However, there is no evidence that a non-linear func- tion of year is needed (p-value = 0.349). 
...
plot.gam(gam.lo, se=TRUE, col="green")  # Does not work
